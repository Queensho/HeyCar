...
class _OwnerChatPageState extends State<OwnerChatPage> {
  ...
  Future<void> _resolve() async {
    ...
    try {
      ...
      final id = d['conversationId']?.toString() ?? '';
      if (id.isEmpty) throw Exception();
      conversationId = id;
      blocked = d['status']?.toString() == 'blocked';
      await _load();
      timer?.cancel();
      if (!blocked) timer = Timer.periodic(const Duration(seconds: 3), (_) => _load(silent: true));
    } catch (_) {
  ...
  Future<void> _load({bool silent = false}) async {
    ...
    try {
      ...
      if (list is! List) throw Exception();
  ...
  Future<void> _block() async {
    ...
    try {
      ...
      if (mounted) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Tarama oturumu engellendi. QR etiketiniz değişmedi.')));
      }
    } catch (_) {
      if (mounted) ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Oturum engellenemedi. Tekrar dene.')));
    }
  }
                  ...
                  children: [
                    if (blocked)
                      Container(
                        ...
                        color: const Color(0x22FF4D63),
                        child: const Row(
                          children: [
                            Icon(Icons.block_rounded, color: _danger, size: 19),
                            SizedBox(width: 8),
                            Expanded(child: Text('Bu tarama oturumu engellendi. Yeni mesaj gönderilemez.', style: TextStyle(color: _danger, fontSize: 12.5, fontWeight: FontWeight.w800))),
                          ],
                        ),
                      ),
                    Expanded(
