...
class _BusinessPanelPageState extends State<BusinessPanelPage>{
 ...
 Widget build(BuildContext context){
  ...
  if(desktop&&tab==3) page=_UsageCodesPage(onVerify:()=>_showVerify(context));
 ...
 Future<void> _showVerify(BuildContext context)async{
  final plate=TextEditingController(),code=TextEditingController();String? error;bool busy=false;
  await showDialog(context:context,builder:(d)=>StatefulBuilder(builder:(d,setD)=>AlertDialog(backgroundColor:_panel,title:const Text('Fırsat Kullanımını Doğrula',style:TextStyle(color:Colors.white,fontWeight:FontWeight.w900)),content:SizedBox(width:420,child:Column(mainAxisSize:MainAxisSize.min,children:[const Text('Müşterinin plakasını ve tek kullanımlık kodunu gir.',style:TextStyle(color:_muted)),const SizedBox(height:14),_Field(plate,'Plaka'),_Field(code,'Kullanım kodu'),if(error!=null)Text(error!,style:const TextStyle(color:Colors.redAccent,fontWeight:FontWeight.w700))])),actions:[TextButton(onPressed:busy?null:()=>Navigator.pop(d),child:const Text('Vazgeç')),FilledButton.icon(onPressed:busy?null:()async{setD(()=>busy=true);try{final r=await http.post(Uri.parse('$_businessApi/api/business/redemptions/verify'),headers:{'Content-Type':'application/json','Authorization':'Bearer $token'},body:jsonEncode({'plate':plate.text.trim(),'code':code.text.trim()}));if(r.statusCode>=200&&r.statusCode<300){if(d.mounted)Navigator.pop(d);await _loadAll();if(mounted)ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content:Text('Fırsat kullanımı onaylandı. Toplam Kullanım güncellendi.')));return;}final body=r.body;if(body.contains('ALREADY_USED'))error='Bu kod daha önce kullanılmış.';else if(body.contains('CODE_NOT_FOUND'))error='Plaka ve kullanım kodu eşleşmedi.';else error='Doğrulama yapılamadı.';}catch(_){error='Sunucuya ulaşılamadı.';}if(d.mounted)setD(()=>busy=false);},icon:busy?const SizedBox(width:16,height:16,child:CircularProgressIndicator(strokeWidth:2,color:Colors.white)):const Icon(Icons.verified_rounded),label:const Text('Onayla'))])));
 }
 ...
 Future<void> _showCampaign(BuildContext context) async {
  final title=TextEditingController(),desc=TextEditingController(),badge=TextEditingController(),code=TextEditingController(),regularPrice=TextEditingController(),offerPrice=TextEditingController(),discountPercent=TextEditingController(); String offerType='discount';
  ...
  await showDialog(context:context,builder:(d)=>StatefulBuilder(builder:(d,setD)=>AlertDialog(
   ...
   content:SizedBox(width:430,child:SingleChildScrollView(child:Column(mainAxisSize:MainAxisSize.min,children:[
    _Field(title,'Kampanya başlığı'),_Field(desc,'Açıklama'),DropdownButtonFormField<String>(initialValue:offerType,dropdownColor:_panel,style:const TextStyle(color:Colors.white),decoration:const InputDecoration(labelText:'Fırsat türü'),items:const [DropdownMenuItem(value:'discount',child:Text('İndirim')),DropdownMenuItem(value:'special_price',child:Text('Özel Fiyat')),DropdownMenuItem(value:'gift',child:Text('Avantaj / Hediye'))],onChanged:(v)=>setD(()=>offerType=v??'discount')),const SizedBox(height:10),if(offerType!='gift')...[_Field(regularPrice,'Normal fiyat (TL)'),_Field(offerPrice,'Cepqar fiyatı (TL)')],if(offerType=='discount')_Field(discountPercent,'İndirim oranı (%)'),_Field(badge,offerType=='gift'?'Avantaj / hediye':'Fırsat etiketi'),_Field(code,'Kampanya kodu'),
   ...
   actions:[TextButton(onPressed:()=>Navigator.pop(d),child:const Text('Vazgeç')),FilledButton(onPressed:()async{
    ...
    final r=await http.post(Uri.parse('$_businessApi/api/business/campaigns'),headers:{'Content-Type':'application/json','Authorization':'Bearer $token'},body:jsonEncode({'title':title.text,'description':desc.text,'badge':badge.text,'couponCode':code.text,'offerType':offerType,'regularPrice':double.tryParse(regularPrice.text.replaceAll(',','.')),'offerPrice':double.tryParse(offerPrice.text.replaceAll(',','.')),'discountPercent':int.tryParse(discountPercent.text),'startsAt':startUtc.toIso8601String(),'endsAt':endUtc.toIso8601String()}));
...
class _UsageCodesPage extends StatelessWidget{const _UsageCodesPage({required this.onVerify});final VoidCallback onVerify;@override Widget build(BuildContext c)=>Column(crossAxisAlignment:CrossAxisAlignment.start,children:[const Text('Kullanım Kodları',style:TextStyle(color:Colors.white,fontSize:25,fontWeight:FontWeight.w900)),const SizedBox(height:6),const Text('Müşterinin plaka + tek kullanımlık kodunu eşleştir.',style:TextStyle(color:_muted)),const SizedBox(height:18),_Box(child:Column(children:[const Icon(Icons.qr_code_scanner_rounded,color:_purple,size:58),const SizedBox(height:12),const Text('Fırsat doğrulama',style:TextStyle(color:Colors.white,fontSize:18,fontWeight:FontWeight.w900)),const SizedBox(height:6),const Text('Doğrulama başarılı olduğunda fırsat kullanılmış olur ve Toplam Kullanım +1 artar.',textAlign:TextAlign.center,style:TextStyle(color:_muted)),const SizedBox(height:16),SizedBox(width:double.infinity,height:48,child:FilledButton.icon(onPressed:onVerify,icon:const Icon(Icons.verified_user_rounded),label:const Text('Plaka + Kod Doğrula')))]))]);}
