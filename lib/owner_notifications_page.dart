...
class _OwnerNotificationsPageState extends State<OwnerNotificationsPage> {
  ...
  void _reply(Map<String, dynamic> item) {
    final id = item['id']?.toString() ?? '';
    if (id.isEmpty) return;
    if (item['status']?.toString() == 'new') _setStatus(item, 'read');
    Navigator.push(
      context,
      MaterialPageRoute(
  ...
  Future<void> _openPhoto(String path) async {
    await launchUrl(Uri.parse(path.startsWith('http') ? path : '$baseUrl$path'), mode: LaunchMode.externalApplication);
  }
    ...
    return Container(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Container(width: 70, height: 70, decoration: BoxDecoration(color: color.withValues(alpha: .16), borderRadius: BorderRadius.circular(14)), child: Icon(_icon(type), color: color, size: 32)),
          ...
          Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Row(children: [
              ...
              if (fresh) ...[const SizedBox(width: 7), const CircleAvatar(radius: 5, backgroundColor: Color(0xFFFF4D63))],
            ]),
            if (message.isNotEmpty) ...[const SizedBox(height: 8), Text(message, maxLines: 3, overflow: TextOverflow.ellipsis, style: const TextStyle(color: Colors.white, fontSize: 14.5, height: 1.3))],
            if (plate.isNotEmpty) ...[const SizedBox(height: 7), Text(plate, style: const TextStyle(color: _muted, fontSize: 12.5))],
          ])),
        ]),
        if (photo.isNotEmpty || (lat != null && lng != null)) ...[
          const SizedBox(height: 12),
          Row(children: [
            if (photo.isNotEmpty) Expanded(child: _ActionButton(icon: Icons.image_outlined, label: 'Fotoğraf', onTap: () => _openPhoto(photo))),
            if (photo.isNotEmpty && lat != null && lng != null) const SizedBox(width: 8),
            if (lat != null && lng != null) Expanded(child: _ActionButton(icon: Icons.location_on_rounded, label: 'Konum', onTap: () => _openMap(lat, lng))),
          ]),
        ],
        ...
        Row(children: [
          Expanded(child: FilledButton.icon(
            ...
            label: const Text('Cevapla', style: TextStyle(fontWeight: FontWeight.w900)),
          )),
          const SizedBox(width: 8),
          if (status != 'resolved') Expanded(child: TextButton(onPressed: () => _setStatus(n, 'resolved'), child: const Text('Çözüldü', style: TextStyle(color: _muted, fontWeight: FontWeight.w800))))
          else const Expanded(child: Text('Çözüldü ✓', textAlign: TextAlign.center, style: TextStyle(color: Color(0xFF38D178), fontWeight: FontWeight.w900))),
        ]),
      ]),
    );
  }
...
class _Tab extends StatelessWidget {
  ...
  Widget build(BuildContext context) => InkWell(
        ...
        child: Container(
          ...
          decoration: BoxDecoration(color: active ? _purple : _panel2, borderRadius: BorderRadius.circular(18), border: Border.all(color: active ? const Color(0xFFB18AFF) : _line)),
          child: Row(mainAxisAlignment: MainAxisAlignment.center, children: [
            ...
            if (badge != null) ...[const SizedBox(width: 5), Container(width: 21, height: 21, alignment: Alignment.center, decoration: const BoxDecoration(color: Color(0xFFFF4D63), shape: BoxShape.circle), child: Text('$badge', style: const TextStyle(color: Colors.white, fontWeight: FontWeight.w900, fontSize: 10.5)))],
          ]),
        ),
      );
}
...
class _ActionButton extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) => SizedBox(height: 40, child: FilledButton.icon(onPressed: onTap, style: FilledButton.styleFrom(backgroundColor: const Color(0xFF4431A7), foregroundColor: Colors.white), icon: Icon(icon, size: 17), label: Text(label, style: const TextStyle(fontWeight: FontWeight.w800, fontSize: 12))));
}
