        ...
        child: Center(
          child: ConstrainedBox(
            ...
            child: SingleChildScrollView(
              padding: EdgeInsets.fromLTRB(h, 12, h, 28),
              child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                _TopBar(onMenu: () => scaffoldKey.currentState?.openEndDrawer()),
                ...
                SizedBox(
                  ...
                  child: Stack(clipBehavior: Clip.none, children: [
                    ...
                    Positioned(
                      right: compact ? -118 : -124,
                      top: compact ? -22 : -26,
                      width: compact ? 305 : 335,
                      height: compact ? 295 : 320,
                ...
                Transform.translate(
                  ...
                  child: Container(
                    ...
                    decoration: BoxDecoration(color: _panel, borderRadius: BorderRadius.circular(21), border: Border.all(color: _line)),
                    child: Stack(clipBehavior: Clip.none, children: [
                      Positioned(left: compact ? 14 : 17, top: compact ? 29 : 32, width: compact ? 170 : 190, child: Row(crossAxisAlignment: CrossAxisAlignment.start, children: [const Icon(Icons.info_outline, color: _purple, size: 21), const SizedBox(width: 9), Expanded(child: Text('Kodu aracın üzerindeki\nHeyCar etiketinde bulabilirsin.', style: TextStyle(color: _muted, fontSize: compact ? 12.5 : 14, height: 1.45)))])),
                      Positioned(right: compact ? -7 : -10, bottom: compact ? -4 : -7, width: compact ? 184 : 208, height: compact ? 111 : 124, child: Transform.rotate(angle: -0.06, child: Image.asset('assets/Qrkod.png', fit: BoxFit.contain, alignment: Alignment.bottomRight))),
                    ]),
                  ),
                ),
                const SizedBox(height: 15),
                Center(child: Text('HeyCar  💜  Yollarda daha fazla bağlantı', style: TextStyle(color: _muted, fontSize: compact ? 12 : 13))),
              ]),
            ),
          ),
        ),
  ...
  Widget build(BuildContext context) => Drawer(
    ...
    child: SafeArea(
      child: Padding(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Row(children: [
            const Text.rich(TextSpan(children: [TextSpan(text: 'Hey', style: TextStyle(color: Colors.white)), TextSpan(text: 'Car', style: TextStyle(color: _purple))]), style: TextStyle(fontSize: 30, fontWeight: FontWeight.w900, letterSpacing: -1.4)),
            const Spacer(),
            IconButton(onPressed: () => Navigator.pop(context), icon: const Icon(Icons.close_rounded, color: Colors.white, size: 28)),
          ]),
          const SizedBox(height: 14),
          const Text('MENÜ', style: TextStyle(color: _muted, fontSize: 12, letterSpacing: 2.4, fontWeight: FontWeight.w800)),
          const SizedBox(height: 12),
          _menuItem(Icons.info_outline_rounded, 'HeyCar Nedir?', onAbout),
...
class _TopBar extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Row(children: [
      Text.rich(TextSpan(children: const [TextSpan(text: 'Hey', style: TextStyle(color: Colors.white)), TextSpan(text: 'Car', style: TextStyle(color: _purple))]), style: TextStyle(fontSize: compact ? 28 : 30, fontWeight: FontWeight.w900, letterSpacing: -1.3)),
      const Spacer(),
