...
Future<void> heyCarNotificationResponseBackground(NotificationResponse response) async {
  WidgetsFlutterBinding.ensureInitialized();
  await PushNotifications.handleResponse(response);
}
...
class PushNotifications {
  ...
  static Future<void> registerToken() async {
    ...
    try{
      final response=await http.post(Uri.parse('$_apiBase/api/owner/push-token'),headers:{'content-type':'application/json','x-owner-id':ownerId},body:jsonEncode({'token':token,'deviceId':deviceId,'platform':Platform.operatingSystem})).timeout(const Duration(seconds:15));
      if(response.statusCode<200 || response.statusCode>=300){throw HttpException('Push token registration failed: ${response.statusCode} ${response.body}');}
  ...
  static Future<void> show(RemoteMessage m) async {
    ...
    final title=m.notification?.title ?? (isCall?'Gelen Araç Araması':'Cepqar');
    ...
    final details=NotificationDetails(android:AndroidNotificationDetails(
      ...
      actions:isCall?<AndroidNotificationAction>[
        const AndroidNotificationAction('CALL_ACCEPT','Kabul Et',showsUserInterface:true,cancelNotification:true),
        const AndroidNotificationAction('CALL_REJECT','Reddet',showsUserInterface:false,cancelNotification:true),
      ]:null,
    ));
  ...
  static Future<void> handleResponse(NotificationResponse response) async {
    ...
    if(response.actionId=='CALL_REJECT'){
      await _callAction(callId,'reject');
      await prefs.setString('last_call_action','reject:$callId');
      return;
    }
    if(response.actionId=='CALL_ACCEPT'){
      await _callAction(callId,'accept');
      await prefs.setString('last_call_action','accept:$callId');
      return;
    }
  ...
  static Future<void> _callAction(String callId,String action) async {
    ...
    if(ownerId==null || ownerId.isEmpty) return;
    try{
      await http.patch(Uri.parse('$_apiBase/api/owner/calls/$callId/$action'),headers:{'content-type':'application/json','x-owner-id':ownerId}).timeout(const Duration(seconds:10));
