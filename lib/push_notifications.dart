...
class PushNotifications {
  ...
  static Future<void> show(RemoteMessage m) async {
    final data=m.data; final type=data['type']??'notification'; final isCall=type=='incoming_call'||type=='call_request'; final plate=(data['plate']??'').toString().trim();
    final serverTitle=m.notification?.title?.trim()??'';
    final title=plate.isNotEmpty ? plate.toUpperCase() : (serverTitle.isNotEmpty?serverTitle:(isCall?'Gelen Araç Araması':'Cepqar'));
    final serverBody=m.notification?.body?.trim()??'';
    final body=serverBody.isNotEmpty?serverBody:(isCall?(plate.isEmpty?'Aracınız için biri sizi arıyor':'$plate için biri sizi arıyor'):(data['body']??data['message']??'Yeni bildiriminiz var.').toString());
    final details=NotificationDetails(android:AndroidNotificationDetails(isCall?_callChannel:_generalChannel,isCall?'Gelen Aramalar':'Cepqar Bildirimleri',channelDescription:isCall?'QR üzerinden gelen arama istekleri':'Mesaj, araç ve sistem bildirimleri',importance:isCall?Importance.max:Importance.high,priority:isCall?Priority.max:Priority.high,category:isCall?AndroidNotificationCategory.call:null,fullScreenIntent:isCall,ongoing:isCall,autoCancel:!isCall,visibility:NotificationVisibility.public,playSound:true,enableVibration:true,icon:'@mipmap/ic_launcher',largeIcon:const DrawableResourceAndroidBitmap('@mipmap/ic_launcher'),styleInformation:BigTextStyleInformation(body,contentTitle:title),actions:isCall?<AndroidNotificationAction>[const AndroidNotificationAction('CALL_ACCEPT','Kabul Et',showsUserInterface:true,cancelNotification:true),const AndroidNotificationAction('CALL_REJECT','Reddet',showsUserInterface:false,cancelNotification:true)]:null));
