...
class _Offer{const _Offer(this.category,this.name,this.description,this.badge,this.distance,this.rating,this.reviews,this.icon,this.tags,{this.id='',this.address='',this.couponCode='',this.openingHours='',this.imageUrl='',this.logoUrl=''});final String category,name,description,badge,distance,rating,reviews,id,address,couponCode,openingHours,imageUrl,logoUrl; List<String> get categories=>category.split(',').map((x)=>x.trim()).where((x)=>x.isNotEmpty).toList();final IconData icon;final List<String> tags;
 factory _Offer.fromApi(Map<String,dynamic> x){final cat=(x['category']??'Servis').toString();final meters=double.tryParse('${x['distance_m']}')??0;final d=meters<1000?'${meters.round()} m':'${(meters/1000).toStringAsFixed(1)} km';IconData icon=Icons.build_rounded;if(cat.toLowerCase().contains('yıkama'))icon=Icons.local_car_wash_rounded;else if(cat.toLowerCase().contains('lastik'))icon=Icons.tire_repair_rounded;else if(cat.toLowerCase().contains('otopark'))icon=Icons.local_parking_rounded;else if(cat.toLowerCase().contains('akaryakıt'))icon=Icons.local_gas_station_rounded;else if(cat.toLowerCase().contains('detail'))icon=Icons.auto_awesome_rounded;return _Offer(cat,(x['name']??'Cepqar İşletmesi').toString(),(x['description']??'').toString(),(x['badge']??'Fırsat').toString(),d,(x['rating']??'—').toString(),(x['review_count']??0).toString(),icon,const [],imageUrl:(x['image_url']??'').toString(),logoUrl:(x['logo_url']??'').toString(),address:(x['address']??'').toString(),couponCode:(x['coupon_code']??'').toString(),openingHours:(x['opening_hours']??'').toString(),id:(x['id']??'').toString());}
}
...
class _OfferDetailPage extends StatefulWidget{
 const _OfferDetailPage({required this.o});final _Offer o;
 @override State<_OfferDetailPage> createState()=>_OfferDetailPageState();
}
class _OfferDetailPageState extends State<_OfferDetailPage>{
 bool favorite=false,busy=false;Map<String,dynamic>? redemption;List<Map<String,dynamic>> storeReviews=[];String storeRating='—';int storeReviewCount=0;
 _Offer get o=>widget.o;
 @override void initState(){super.initState();_loadState();}
 Future<Map<String,String>> _ownerHeaders()async=>OwnerAuth.headers();
 Future<({String owner,String plate})> _ownerVehicle()async{final p=await SharedPreferences.getInstance();final owner=OnboardingDraft.userId.trim().isNotEmpty?OnboardingDraft.userId.trim():(p.getString('owner_user_id')??'').trim();var plate=(p.getString('owner_plate')??'').trim().toUpperCase();if(owner.isEmpty)return(owner:'',plate:plate);try{final r=await OwnerHttp.get(Uri.parse('${OnboardingBackend.baseUrl}/api/owner/vehicles'),json:false).timeout(const Duration(seconds:10));if(r.statusCode>=200&&r.statusCode<300){final d=jsonDecode(r.body);if(d is Map&&d['vehicles'] is List){final vs=(d['vehicles'] as List).whereType<Map>().toList();if(vs.isNotEmpty){final selected=(p.getString('owner_vehicle_id')??'').trim();Map v=vs.first;for(final x in vs){if('${x['id']??''}'==selected){v=x;break;}}plate='${v['plate']??''}'.trim().toUpperCase();if(plate.isNotEmpty)await p.setString('owner_plate',plate);}}}}catch(_){}return(owner:owner,plate:plate);}
 Future<void> _loadState()async{if(o.id.isEmpty)return;try{await _loadReviews();if(OnboardingDraft.userId.trim().isEmpty)return;final rs=await Future.wait([OwnerHttp.get(Uri.parse('${_CepqarOffersPageState.apiBase}/api/offers/favorites'),json:false),OwnerHttp.get(Uri.parse('${_CepqarOffersPageState.apiBase}/api/offers/${o.id}/redemption'),json:false)]);if(!mounted)return;final f=jsonDecode(rs[0].body),r=jsonDecode(rs[1].body);setState((){favorite=f is Map&&f['favorites'] is List&&(f['favorites'] as List).map((e)=>e.toString()).contains(o.id);redemption=r is Map&&r['redemption'] is Map?Map<String,dynamic>.from(r['redemption']):null;});}catch(_){}}
 Future<void> _loadReviews()async{try{final r=await http.get(Uri.parse('${_CepqarOffersPageState.apiBase}/api/offers/${o.id}/reviews'));if(r.statusCode==200){final d=jsonDecode(r.body);if(d is Map&&mounted)setState((){storeReviews=(d['reviews'] as List? ?? []).whereType<Map>().map((e)=>Map<String,dynamic>.from(e)).toList();storeRating=(d['rating']??'—').toString();storeReviewCount=int.tryParse('${d['reviewCount']??0}')??0;});}}catch(_){}}
 Future<void> _toggleFavorite()async{if(o.id.isEmpty)return;if(OnboardingDraft.userId.trim().isEmpty){_msg('Favori için giriş yapmalısın.');return;}final next=!favorite;setState(()=>favorite=next);try{final u=Uri.parse('${_CepqarOffersPageState.apiBase}/api/offers/${o.id}/favorite');final r=next?await OwnerHttp.post(u):await OwnerHttp.delete(u);if(r.statusCode<200||r.statusCode>=300)throw Exception();}catch(_){if(mounted)setState(()=>favorite=!next);_msg('Favori kaydedilemedi.');}}
 Future<void> _share()async{final link='https://queensho.github.io/HeyCar/?offer=${Uri.encodeComponent(o.id)}';await SharePlus.instance.share(ShareParams(title:'Cepqar Fırsatı',text:'${o.name} • ${o.badge}\n${o.description}\n$link'));}
 Future<void> _useOffer()async{if(busy||o.id.isEmpty)return;final session=await _ownerVehicle(),owner=session.owner,plate=session.plate;if(owner.isEmpty){_msg('Fırsatı kullanmak için giriş yapmalısın.');return;}if(plate.isEmpty){_msg('Önce hesabına bir araç/plaka eklemelisin.');return;}setState(()=>busy=true);try{final r=await OwnerHttp.post(Uri.parse('${_CepqarOffersPageState.apiBase}/api/offers/${o.id}/redeem'),body:jsonEncode({'plate':plate}));final d=jsonDecode(r.body);if(r.statusCode<200||r.statusCode>=300||d is! Map||d['redemption'] is! Map)throw Exception();redemption=Map<String,dynamic>.from(d['redemption']);if(mounted){setState((){});_showCode();}}catch(_){_msg('Kullanım kodu oluşturulamadı.');}finally{if(mounted)setState(()=>busy=false);}}
 void _showCode(){final r=redemption;if(r==null)return;final code='${r['usage_code']??''}',plate='${r['plate']??''}';showModalBottomSheet(context:context,backgroundColor:CepqarTheme.panel,isScrollControlled:true,builder:(c)=>SafeArea(child:Padding(padding:const EdgeInsets.all(22),child:Column(mainAxisSize:MainAxisSize.min,children:[Text(r['status']=='redeemed'?'Fırsat Kullanıldı':'İşletmeye Göster',style:TextStyle(color:CepqarTheme.text,fontSize:21,fontWeight:FontWeight.w900)),const SizedBox(height:8),Text('Plaka: $plate',style:TextStyle(color:CepqarTheme.muted,fontWeight:FontWeight.w700)),const SizedBox(height:16),if(code.isNotEmpty)Container(width:210,height:210,padding:const EdgeInsets.all(10),decoration:BoxDecoration(color:Colors.white,borderRadius:BorderRadius.circular(18)),child:Image.network('https://quickchart.io/qr?text=${Uri.encodeComponent(code)}&size=420',errorBuilder:(_,__,___)=>const Icon(Icons.qr_code_2,color:Colors.black,size:150))),const SizedBox(height:12),SelectableText(code,style:TextStyle(color:CepqarTheme.text,fontSize:25,fontWeight:FontWeight.w900,letterSpacing:3)),const SizedBox(height:8),Text('İşletme plaka + bu kodu doğruladıktan sonra fırsat kullanılmış sayılır.',textAlign:TextAlign.center,style:TextStyle(color:CepqarTheme.muted,fontSize:11))]))));}
 Future<void> _review()async{int rating=5;final tc=TextEditingController();final ok=await showDialog<bool>(context:context,builder:(d)=>StatefulBuilder(builder:(d,setD)=>AlertDialog(backgroundColor:CepqarTheme.panel,title:Text('Mağazayı Değerlendir',style:TextStyle(color:CepqarTheme.text)),content:Column(mainAxisSize:MainAxisSize.min,children:[Row(mainAxisAlignment:MainAxisAlignment.center,children:List.generate(5,(i)=>IconButton(onPressed:()=>setD(()=>rating=i+1),icon:Icon(i<rating?Icons.star_rounded:Icons.star_border_rounded,color:const Color(0xFFFFB72B)))),),TextField(controller:tc,maxLines:3,style:TextStyle(color:CepqarTheme.text),decoration:const InputDecoration(hintText:'Yorumun'))]),actions:[TextButton(onPressed:()=>Navigator.pop(d,false),child:const Text('Vazgeç')),FilledButton(onPressed:()=>Navigator.pop(d,true),child:const Text('Gönder'))])));if(ok!=true)return;final r=await OwnerHttp.post(Uri.parse('${_CepqarOffersPageState.apiBase}/api/offers/${o.id}/review'),body:jsonEncode({'rating':rating,'comment':tc.text.trim()}));_msg(r.statusCode>=200&&r.statusCode<300?'Mağaza değerlendirmen kaydedildi.':'Değerlendirme kaydedilemedi.');if(r.statusCode>=200&&r.statusCode<300)await _loadReviews();}
 void _msg(String s){if(mounted)ScaffoldMessenger.of(context).showSnackBar(SnackBar(content:Text(s)));}
 @override Widget build(BuildContext context)=>Scaffold(
  backgroundColor:CepqarTheme.bg,
  appBar:AppBar(backgroundColor:CepqarTheme.bg,foregroundColor:CepqarTheme.text,elevation:0,title:const Text('Fırsat Detayı',style:TextStyle(fontSize:18,fontWeight:FontWeight.w900)),actions:[IconButton(onPressed:_toggleFavorite,icon:Icon(favorite?Icons.favorite_rounded:Icons.favorite_border_rounded,size:22,color:favorite?Colors.redAccent:null)),IconButton(onPressed:_share,icon:const Icon(Icons.share_outlined,size:21))]),
  bottomNavigationBar:SafeArea(child:Padding(padding:const EdgeInsets.fromLTRB(16,8,16,10),child:SizedBox(height:48,child:FilledButton.icon(onPressed:busy?null:(redemption?['status']=='redeemed'?_review:(redemption!=null?_showCode:_useOffer)),style:FilledButton.styleFrom(backgroundColor:CepqarTheme.purple,shape:RoundedRectangleBorder(borderRadius:BorderRadius.circular(16))),icon:const Icon(Icons.local_offer_rounded,size:20),label:Text(redemption?['status']=='redeemed'?'Puan ve Yorum Yap':(redemption!=null?'Kullanım Kodunu Göster':'Bu Fırsatı Kullan'),style:const TextStyle(fontSize:14,fontWeight:FontWeight.w900)))))),
  body:ListView(padding:const EdgeInsets.fromLTRB(16,4,16,18),children:[
   Container(height:190,decoration:BoxDecoration(gradient:const LinearGradient(colors:[Color(0xFF121B3B),Color(0xFF4420A8)]),borderRadius:BorderRadius.circular(20)),clipBehavior:Clip.antiAlias,child:Stack(children:[Positioned.fill(child:o.imageUrl.isNotEmpty?Image.network(o.imageUrl,fit:BoxFit.cover,errorBuilder:(_,__,___)=>Center(child:Icon(o.icon,color:Colors.white,size:82))):Center(child:Icon(o.icon,color:Colors.white,size:82))),Positioned(left:12,top:12,child:_Badge(o.badge)),Positioned(right:12,bottom:10,child:Container(padding:const EdgeInsets.symmetric(horizontal:9,vertical:5),decoration:BoxDecoration(color:Colors.black.withValues(alpha:.45),borderRadius:BorderRadius.circular(12)),child:const Text('1/5',style:TextStyle(color:Colors.white,fontSize:11))))])),
   ...
   const SizedBox(height:6),Row(children:[const Icon(Icons.star_rounded,color:Color(0xFFFFB72B),size:19),const SizedBox(width:4),Text('${storeReviewCount>0?storeRating:o.rating} (${storeReviewCount>0?storeReviewCount:o.reviews} mağaza değerlendirmesi)',style:TextStyle(color:CepqarTheme.muted,fontSize:12)),const Spacer(),_Pill(_isOpenNow(o.openingHours)?'Açık':'Kapalı',_isOpenNow(o.openingHours)?const Color(0xFF31D47B):Colors.redAccent)]),
   ...
   _Title('Mağaza Yorumları'),const SizedBox(height:8),if(storeReviews.isEmpty)_InfoCard(child:Text('Henüz mağaza değerlendirmesi yok.',style:TextStyle(color:CepqarTheme.muted,fontSize:11)))else ...storeReviews.map((r)=>Padding(padding:const EdgeInsets.only(bottom:8),child:_InfoCard(child:Row(crossAxisAlignment:CrossAxisAlignment.start,children:[const CircleAvatar(radius:18,child:Icon(Icons.person,size:18)),const SizedBox(width:10),Expanded(child:Column(crossAxisAlignment:CrossAxisAlignment.start,children:[Text('Cepqar kullanıcısı  ${'★'*(int.tryParse('${r['rating']}')??0)}',style:TextStyle(color:CepqarTheme.text,fontSize:11.5,fontWeight:FontWeight.w800)),if('${r['comment']??''}'.trim().isNotEmpty)...[const SizedBox(height:4),Text('${r['comment']}',style:TextStyle(color:CepqarTheme.muted,fontSize:10.5))]]))]))))
  ])
 );
}
