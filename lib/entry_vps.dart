...
void main() => runApp(const EntryVpsApp());

class EntryVpsApp extends StatelessWidget {
  const EntryVpsApp({super.key});
  ...
  Widget build(BuildContext context) => MaterialApp(
        ...
        home: const OnboardingVps(),
      );
}

class OnboardingVps extends StatefulWidget {
  const OnboardingVps({super.key});

  @override
  State<OnboardingVps> createState() => _OnboardingVpsState();
}

class _OnboardingVpsState extends State<OnboardingVps> {
  int index = 0;
  ...
  Widget build(BuildContext context) => [
        ...
        PhoneVps(next, back),
        old.Otp(next, back),
        AccountVps(next, back),
        VehiclePickerVps(next, back),
...
class PhoneVps extends StatefulWidget {
  const PhoneVps(this.next, this.back, {super.key});
  final VoidCallback next, back;

  @override
  State<PhoneVps> createState() => _PhoneVpsState();
}

class _PhoneVpsState extends State<PhoneVps> {
  late final TextEditingController phone;

  @override
  void initState() {
    super.initState();
    phone = TextEditingController(text: OnboardingDraft.phone);
  }
              ...
              children: [
                ...
                Expanded(
                  child: TextField(
                    ...
                    decoration: const InputDecoration(hintText: '532 123 45 67'),
                  ),
                ),
              ],
            ),
            const SizedBox(height: 18),
            old.Primary('Doğrulama kodu gönder', () {
              OnboardingDraft.phone = phone.text.trim();
              widget.next();
            }),
          ],
        ),
...
class AccountVps extends StatefulWidget {
  const AccountVps(this.next, this.back, {super.key});
  final VoidCallback next, back;

  @override
  State<AccountVps> createState() => _AccountVpsState();
}

class _AccountVpsState extends State<AccountVps> {
  ...
  void initState() {
    super.initState();
    name = TextEditingController(text: OnboardingDraft.displayName);
    email = TextEditingController(text: OnboardingDraft.email);
    password = TextEditingController(text: OnboardingDraft.password);
  }
  ...
  Widget build(BuildContext context) => old.Frame(
        ...
        Column(
          children: [
            ...
            old.Primary('Devam et', () {
              if (name.text.trim().isEmpty || password.text.length < 6) {
                ScaffoldMessenger.of(context).showSnackBar(
                  const SnackBar(content: Text('Ad soyad ve en az 6 karakterli şifre gerekli.')),
                );
                return;
              }
...
class VehiclePickerVps extends StatefulWidget {
  const VehiclePickerVps(this.next, this.back, {super.key});
  final VoidCallback next, back;

  @override
  State<VehiclePickerVps> createState() => _VehiclePickerVpsState();
}

class _VehiclePickerVpsState extends State<VehiclePickerVps> {
  ...
  Future<void> _save() async {
    ...
    if (OnboardingDraft.displayName.isEmpty || OnboardingDraft.password.length < 6) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Hesap bilgilerini tamamlamalısın.')),
      );
      return;
    }
