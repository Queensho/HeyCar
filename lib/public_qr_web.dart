...
class _PrivacyFooter extends StatelessWidget {
  ...
  @override Widget build(BuildContext context) => const SizedBox(height: 30, child: Row(mainAxisAlignment: MainAxisAlignment.center, children: [
    Icon(Icons.verified_user_rounded, color: Colors.white, size: 22), SizedBox(width: 9), Text('Kişisel bilgileriniz gizli kalır.', style: TextStyle(color: Color(0xFFBFC7D1), fontSize: 13.2)),
  ]));
}

class _MaskedCallScreen extends StatefulWidget {
  const _MaskedCallScreen();
  @override State<_MaskedCallScreen> createState() => _MaskedCallScreenState();
}

class _MaskedCallScreenState extends State<_MaskedCallScreen> with SingleTickerProviderStateMixin {
  bool muted = false, speaker = false;
  int seconds = 0;
  Timer? _timer;
  late final AnimationController _pulse;

  String get timer => '${(seconds ~/ 60).toString().padLeft(2, '0')}:${(seconds % 60).toString().padLeft(2, '0')}';

  @override
  void initState() {
    super.initState();
    _pulse = AnimationController(vsync: this, duration: const Duration(milliseconds: 1900))..repeat();
      ...
      SafeArea(child: LayoutBuilder(builder: (context, c) {
        ...
        return Center(child: ConstrainedBox(constraints: const BoxConstraints(maxWidth: 430), child: Padding(
          ...
          child: Column(children: [
            ...
            Text(timer, style: const TextStyle(color: Colors.white, fontSize: 31)),
            SizedBox(height: compact ? 28 : 44),
            _PulseCar(animation: _pulse, compact: compact),
...
class _PulseCar extends StatelessWidget {
  const _PulseCar({required this.animation, required this.compact});
  final Animation<double> animation;
  final bool compact;

  @override
  Widget build(BuildContext context) {
    final base = compact ? 150.0 : 176.0;
    final size = compact ? 250.0 : 286.0;
    return SizedBox(
      width: size,
      height: size,
      child: AnimatedBuilder(
        ...
        builder: (context, child) {
          Widget wave(double offset) {
            final t = (animation.value + offset) % 1.0;
            final d = base + (size - base) * t;
            final opacity = (1.0 - t) * .34;
            return Container(
              width: d,
              height: d,
              decoration: BoxDecoration(
                shape: BoxShape.circle,
                border: Border.all(color: _orange.withValues(alpha: opacity), width: 3),
                boxShadow: [BoxShadow(color: _orange.withValues(alpha: opacity * .45), blurRadius: 18, spreadRadius: 2)],
              ),
            );
          }
          ...
          return Stack(alignment: Alignment.center, children: [
            ...
            Container(
              ...
              decoration: BoxDecoration(
                shape: BoxShape.circle,
                color: const Color(0x221D2734),
                border: Border.all(color: _orange, width: 7),
                boxShadow: [BoxShadow(color: _orange.withValues(alpha: .30), blurRadius: 26, spreadRadius: 3)],
              ),
...
class _MessageScreen extends StatefulWidget {
  ...
  @override State<_MessageScreen> createState() => _MessageScreenState();
}
...
class _MessageScreenState extends State<_MessageScreen> {
  ...
  @override Widget build(BuildContext context) => Scaffold(backgroundColor: _bg, body: Stack(fit: StackFit.expand, children: [
    ...
    SafeArea(child: Center(child: ConstrainedBox(constraints: const BoxConstraints(maxWidth: 430), child: Column(children: [
      Padding(padding: const EdgeInsets.fromLTRB(14,8,14,2), child: Row(children: [
        IconButton(onPressed: ()=>Navigator.pop(context), icon: const Icon(Icons.chevron_left_rounded,color:Colors.white,size:36)),
        const Expanded(child: Text('Mesaj Gönder',textAlign:TextAlign.center,style:TextStyle(color:Colors.white,fontSize:23,fontWeight:FontWeight.w800))), const SizedBox(width:50),
      ])),
      ...
      const SizedBox(height:10), const Text('34 ABC 123',style:TextStyle(color:Colors.white,fontSize:27,fontWeight:FontWeight.w900)),
      const SizedBox(height:4), const Text('Araç sahibine anonim mesaj\ngönderilecektir.',textAlign:TextAlign.center,style:TextStyle(color:Colors.white70,fontSize:15.5,height:1.3)), const SizedBox(height:18),
      Expanded(child: Container(width:double.infinity,padding:const EdgeInsets.fromLTRB(18,20,18,16),decoration:const BoxDecoration(color:Colors.white,borderRadius:BorderRadius.vertical(top:Radius.circular(30))),child:SingleChildScrollView(child:Column(children:[
        ...
        TextField(controller:controller,maxLength:120,maxLines:4,decoration:InputDecoration(hintText:'Araç sahibine mesaj yaz...',filled:true,fillColor:const Color(0xFFF5F6F8),border:OutlineInputBorder(borderRadius:BorderRadius.circular(18),borderSide:BorderSide.none))),
...
class _AttachBox extends StatelessWidget {
  const _AttachBox({required this.icon,required this.title,required this.subtitle,required this.color});
  final IconData icon; final String title,subtitle; final Color color;
  @override Widget build(BuildContext context)=>Container(height:108,decoration:BoxDecoration(color:const Color(0xFFF5F6F8),borderRadius:BorderRadius.circular(18)),child:Column(mainAxisAlignment:MainAxisAlignment.center,children:[
    Icon(icon,color:color,size:32),const SizedBox(height:7),Text(title,style:const TextStyle(fontWeight:FontWeight.w700)),Text(subtitle,style:const TextStyle(color:Color(0xFF667085),fontSize:12.5))
  ]));
}
...
class _MessageSentScreen extends StatelessWidget {
  ...
  @override Widget build(BuildContext context)=>Scaffold(backgroundColor:_bg,body:Stack(fit:StackFit.expand,children:[
    Image.asset('assets/Arka2.png',fit:BoxFit.cover),const ColoredBox(color:Color(0xCC06101B)),
    SafeArea(child:Center(child:ConstrainedBox(constraints:const BoxConstraints(maxWidth:430),child:Column(children:[
      ...
      Container(width:88,height:88,decoration:const BoxDecoration(color:Color(0xAA1B2430),shape:BoxShape.circle),child:const Icon(Icons.send_rounded,color:_orange,size:46)),
      const SizedBox(height:14),const Text('Mesajınız gönderildi!',style:TextStyle(color:Colors.white,fontSize:28,fontWeight:FontWeight.w900)),const SizedBox(height:9),
      ...
      Expanded(child:Container(width:double.infinity,padding:const EdgeInsets.fromLTRB(24,22,24,18),decoration:const BoxDecoration(color:Colors.white,borderRadius:BorderRadius.vertical(top:Radius.circular(30))),child:Column(children:[
        const _StatusStep(icon:Icons.check_rounded,iconColor:Color(0xFF159A8C),title:'Mesaj gönderildi',subtitle:'Şimdi',line:true),
        const _StatusStep(icon:Icons.circle,iconColor:_orange,title:'Araç sahibine iletildi',subtitle:'Bildirim gönderildi',line:true),
        ...
        const SizedBox(height:12),
        SizedBox(width:double.infinity,height:54,child:OutlinedButton.icon(onPressed:()=>_openMessageScreen(context,'Diğer mesaj'),icon:const Icon(Icons.chat_bubble_outline_rounded),label:const Text('Yeni mesaj gönder',style:TextStyle(fontSize:16.5,fontWeight:FontWeight.w800)))),
        const SizedBox(height:8),TextButton(onPressed:()=>Navigator.of(context).popUntil((r)=>r.isFirst),child:const Text('Ana sayfaya dön',style:TextStyle(color:Color(0xFF101828),decoration:TextDecoration.underline,fontWeight:FontWeight.w700)))
      ])))
    ]))))
  ]));
}
...
class _StatusStep extends StatelessWidget {
  ...
  final IconData icon; final Color iconColor; final String title,subtitle; final bool line;
  @override Widget build(BuildContext context)=>Row(crossAxisAlignment:CrossAxisAlignment.start,children:[
    ...
    const SizedBox(width:8),Expanded(child:Padding(padding:const EdgeInsets.only(top:1,bottom:16),child:Column(crossAxisAlignment:CrossAxisAlignment.start,children:[Text(title,style:const TextStyle(fontSize:17,fontWeight:FontWeight.w800,color:Color(0xFF172033))),const SizedBox(height:3),Text(subtitle,style:const TextStyle(fontSize:14,color:Color(0xFF7B8492)))])))
  ]);
}
