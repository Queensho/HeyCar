...
String _auditActionLabel(String action)=>switch(action){
  ...
  'promo.updated'=>'Promo güncellendi',
  'promo.push_sent'=>'Promo bildirimi gönderildi',
  'promo.deactivated'=>'Promo pasifleştirildi',
  _=>action,
};

IconData _auditActionIcon(String action){
  if(action.startsWith('user.'))return Icons.person_rounded;
  if(action.startsWith('qr.')||action.startsWith('qr_batch.'))return Icons.qr_code_2_rounded;
  if(action.startsWith('correction.'))return Icons.support_agent_rounded;
  if(action.startsWith('moderation.'))return Icons.shield_rounded;
  if(action.startsWith('promo.'))return Icons.campaign_rounded;
  return Icons.history_rounded;
...
Color _auditActionColor(String action){
  if(action.contains('suspended')||action.contains('disabled')||action.contains('rejected'))return Colors.redAccent;
  if(action.contains('activated')||action.contains('enabled')||action.contains('resolved')||action.contains('applied'))return _green;
  if(action.contains('print')||action.contains('batch'))return _amber;
  if(action.startsWith('promo.'))return _pink;
  if(action.startsWith('moderation.'))return _blue;
  return _purple;
}
...
String _prettyJson(dynamic value){
  ...
  try{return const JsonEncoder.withIndent('  ').convert(value);}catch(_){return value.toString();}
}

class AuditLogPage extends StatefulWidget{
  const AuditLogPage({super.key,required this.data,required this.loading,required this.error,required this.onRefresh});
  final Map<String,dynamic> data;
  final bool loading;
  final String? error;
...
class _AuditLogPageState extends State<AuditLogPage>{
  final search=TextEditingController();
  String action='all',targetType='all';
  @override void dispose(){search.dispose();super.dispose();}

  List<Map<String,dynamic>> get allRows=>_reportRows(widget.data['items']);

  List<Map<String,dynamic>> get rows{
    final q=search.text.trim().toLowerCase();
    return allRows.where((e){
      final a=(e['action']??'').toString(),t=(e['target_type']??'').toString();
      if(action!='all'&&a!=action)return false;
      if(targetType!='all'&&t!=targetType)return false;
      if(q.isEmpty)return true;
      final hay=[
        e['admin_name'],e['admin_email'],e['admin_id'],e['target_label'],e['target_id'],
        a,_auditActionLabel(a),t,_auditTargetLabel(t),
  ...
  List<String> get actions{
    final raw=widget.data['actions'];
    final out=raw is List?raw.map((x)=>x.toString()).where((x)=>x.isNotEmpty).toList():
      allRows.map((e)=>(e['action']??'').toString()).where((x)=>x.isNotEmpty).toSet().toList();
    out.sort();return out;
  }
  ...
  List<String> get targetTypes{
    final raw=widget.data['targetTypes'];
    final out=raw is List?raw.map((x)=>x.toString()).where((x)=>x.isNotEmpty).toList():
      allRows.map((e)=>(e['target_type']??'').toString()).where((x)=>x.isNotEmpty).toSet().toList();
    out.sort();return out;
  }
  ...
  @override Widget build(BuildContext context){
    ...
    final visible=rows;
    final total=_reportInt(widget.data['total']);
    final today=DateTime.now();
    final todayCount=allRows.where((e){
      final d=DateTime.tryParse((e['created_at']??'').toString())?.toLocal();
      return d!=null&&d.year==today.year&&d.month==today.month&&d.day==today.day;
    }).length;
    final adminCount=allRows.map((e)=>(e['admin_email']??e['admin_name']??e['admin_id']??'').toString()).where((x)=>x.isNotEmpty).toSet().length;
      ...
      child:ListView(
        ...
        children:[
            ...
            child:Column(crossAxisAlignment:CrossAxisAlignment.start,children:[
              Row(children:[
                ...
                const Expanded(child:Column(crossAxisAlignment:CrossAxisAlignment.start,children:[
                  Text('Admin İşlem Geçmişi',style:TextStyle(fontSize:21,fontWeight:FontWeight.w900)),
                  SizedBox(height:2),
                  Text('Kim, hangi kayıtta, ne değiştirdi?',style:TextStyle(color:_muted,fontSize:12)),
                ])),
                if(widget.loading)const SizedBox(width:18,height:18,child:CircularProgressIndicator(strokeWidth:2,color:_purple)),
              ]),
              const SizedBox(height:14),
              Wrap(spacing:8,runSpacing:8,children:[
                _adminStat('$total','Toplam kayıt',Icons.receipt_long_rounded,color:_purple),
                _adminStat('$todayCount','Bugünkü işlem',Icons.today_rounded,color:_blue),
                _adminStat('$adminCount','Admin',Icons.admin_panel_settings_rounded,color:_green),
              ]),
            ]),
          ),
          const SizedBox(height:12),
          TextField(
            controller:search,
            onChanged:(_)=>setState((){}),
            decoration:const InputDecoration(labelText:'Admin, hedef veya işlem ara',prefixIcon:Icon(Icons.search_rounded)),
          ),
          const SizedBox(height:9),
          LayoutBuilder(builder:(context,b){
            final first=DropdownButtonFormField<String>(
              initialValue:actions.contains(action)?action:'all',
              isExpanded:true,
              decoration:const InputDecoration(labelText:'İşlem türü'),
              items:[
                const DropdownMenuItem(value:'all',child:Text('Tüm işlemler')),
                ...actions.map((x)=>DropdownMenuItem(value:x,child:Text(_auditActionLabel(x),overflow:TextOverflow.ellipsis))),
              ],
              onChanged:(v)=>setState(()=>action=v??'all'),
            );
            final second=DropdownButtonFormField<String>(
              initialValue:targetTypes.contains(targetType)?targetType:'all',
              isExpanded:true,
              decoration:const InputDecoration(labelText:'Hedef türü'),
              items:[
                const DropdownMenuItem(value:'all',child:Text('Tüm hedefler')),
                ...targetTypes.map((x)=>DropdownMenuItem(value:x,child:Text(_auditTargetLabel(x),overflow:TextOverflow.ellipsis))),
              ],
              onChanged:(v)=>setState(()=>targetType=v??'all'),
            );
            return b.maxWidth<620
              ? Column(children:[first,const SizedBox(height:9),second])
              : Row(children:[Expanded(child:first),const SizedBox(width:9),Expanded(child:second)]);
          }),
          const SizedBox(height:12),
          Text(visible.length.toString()+' kayıt gösteriliyor',style:const TextStyle(fontWeight:FontWeight.w900)),
          const SizedBox(height:8),
          if(visible.isEmpty)
            _adminEmpty('Bu filtrelere uygun işlem kaydı yok.')
          else
            ...visible.map((e)=>_card(context,e)),
        ],
      ),
    );
  }
  ...
  Widget _card(BuildContext context,Map<String,dynamic> e){
    final a=(e['action']??'').toString(),color=_auditActionColor(a);
    final admin=(e['admin_name']??'').toString().trim().isNotEmpty
      ? e['admin_name'].toString()
      : (e['admin_email']??e['admin_id']??'Admin').toString();
    final type=(e['target_type']??'').toString();
    final target=(e['target_label']??e['target_id']??'-').toString();
    return Container(
      ...
      child:InkWell(
        ...
        child:Padding(
          ...
          child:Row(crossAxisAlignment:CrossAxisAlignment.start,children:[
            Container(width:42,height:42,decoration:BoxDecoration(color:color.withValues(alpha:.13),shape:BoxShape.circle),child:Icon(_auditActionIcon(a),color:color,size:21)),
            const SizedBox(width:10),
            Expanded(child:Column(crossAxisAlignment:CrossAxisAlignment.start,children:[
              Text(_auditActionLabel(a),style:const TextStyle(fontSize:13.5,fontWeight:FontWeight.w900)),
              const SizedBox(height:3),
              Text(admin+' • '+_auditTargetLabel(type),style:const TextStyle(color:_muted,fontSize:11.5,fontWeight:FontWeight.w700)),
              const SizedBox(height:3),
              Text(target,maxLines:2,overflow:TextOverflow.ellipsis,style:const TextStyle(color:Colors.white70,fontSize:11.5)),
            ])),
            const SizedBox(width:8),
            Column(crossAxisAlignment:CrossAxisAlignment.end,children:[
              Text(_adminDate(e['created_at']),style:const TextStyle(color:_muted,fontSize:9.5)),
              const SizedBox(height:8),
  ...
  void _detail(BuildContext context,Map<String,dynamic> e){
    final a=(e['action']??'').toString();
    final admin=(e['admin_name']??'').toString().trim().isNotEmpty
      ? e['admin_name'].toString()
      : (e['admin_email']??e['admin_id']??'Admin').toString();
    showModalBottomSheet(
      ...
      builder:(context)=>DraggableScrollableSheet(
        ...
        builder:(context,scroll)=>ListView(
          controller:scroll,padding:const EdgeInsets.fromLTRB(16,14,16,28),
          children:[
            Center(child:Container(width:42,height:4,decoration:BoxDecoration(color:_muted.withValues(alpha:.4),borderRadius:BorderRadius.circular(8)))),
            const SizedBox(height:16),
            Text(_auditActionLabel(a),style:const TextStyle(fontSize:19,fontWeight:FontWeight.w900)),
            const SizedBox(height:10),
            _info('Admin',admin),
            _info('Admin e-posta',e['admin_email']),
            _info('Zaman',_adminDate(e['created_at'])),
            _info('Hedef',e['target_label']??e['target_id']),
            _info('Hedef türü',_auditTargetLabel((e['target_type']??'').toString())),
            _info('İşlem kodu',a),
            const SizedBox(height:10),
            _json('Önceki durum',e['before_state']),
            const SizedBox(height:9),
            _json('Sonraki durum',e['after_state']),
            const SizedBox(height:9),
            _json('Ek bilgiler',e['metadata']),
          ],
        ),
      ),
    );
  ...
  Widget _info(String label,dynamic value)=>Padding(
    ...
    child:Row(crossAxisAlignment:CrossAxisAlignment.start,children:[
      SizedBox(width:110,child:Text(label,style:const TextStyle(color:_muted,fontSize:11,fontWeight:FontWeight.w700))),
      Expanded(child:Text((value??'-').toString(),style:const TextStyle(fontSize:11.5,fontWeight:FontWeight.w800))),
    ]),
  );

  Widget _json(String title,dynamic value)=>Container(
    width:double.infinity,padding:const EdgeInsets.all(12),
    decoration:BoxDecoration(color:_card2,borderRadius:BorderRadius.circular(14),border:Border.all(color:_line)),
    child:Column(crossAxisAlignment:CrossAxisAlignment.start,children:[
      Text(title,style:const TextStyle(color:Color(0xFFC879FF),fontSize:11.5,fontWeight:FontWeight.w900)),
      const SizedBox(height:7),
      SelectableText(_prettyJson(value),style:const TextStyle(color:Colors.white70,fontSize:10.5,height:1.35,fontFamily:'monospace')),
    ]),
  );
}
