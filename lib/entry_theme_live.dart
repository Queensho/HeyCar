...
class _ThemeOwnerEntryState extends State<ThemeOwnerEntry> with WidgetsBindingObserver{int index=0;bool loginMode=false,registerMode=false,restoring=true,hasSession=false,callPermissionSetupDone=false,runtimeLoading=true;String driverId='';RuntimeAppConfig? runtimeConfig;Widget ownerHome()=>const OwnerCallWatcher(child:OwnerDashboardLive());@override void initState(){super.initState();WidgetsBinding.instance.addObserver(this);PushNotifications.onNavigationRequested=_routePush;_loadRuntimeConfig();_restoreSession();}@override void didChangeAppLifecycleState(AppLifecycleState state){if(state==AppLifecycleState.resumed){_loadRuntimeConfig(silent:true);PushNotifications.refreshTokenRegistration();Future.delayed(const Duration(milliseconds:250),_openPendingPush);}}
Future<void> _loadRuntimeConfig({bool silent=false})async{
  if(!silent&&mounted)setState(()=>runtimeLoading=true);
  try{
    final cfg=await RuntimeConfigService.load();
    if(mounted)setState(()=>runtimeConfig=cfg);
  }catch(e,st){
    debugPrint('Runtime config failed: $e');
    await CrashReporting.record(e,st,reason:'runtime_config');
  }finally{
    if(!silent&&mounted)setState(()=>runtimeLoading=false);
  }
}
Future<void> _restoreSession()async{
  try{
    await Future.wait([OwnerAuth.restore(),DriverAuth.restore()]);
    final prefs=await SharedPreferences.getInstance();
    final driverLogged=(prefs.getBool('driver_logged_in')??false)&&DriverAuth.refreshToken.isNotEmpty;
    driverId=driverLogged?(prefs.getString('driver_user_id')??''):'';

    final ownerFlag=prefs.getBool('owner_logged_in')??false;
    var loggedIn=false;
    ...
    if(ownerFlag){
      ...
      if(!loggedIn){
        ...
        await prefs.setString('push_token_last_error','OWNER_RELOGIN_REQUIRED');
        for(final key in <String>[
          ...
          'owner_model',
          'owner_qr_token',
          'pending_push_navigation',
          'pending_incoming_call_id',
          'pending_incoming_call_auto_accept',
        ]){
    ...
    callPermissionSetupDone=prefs.getBool('call_permission_setup_done')??false;
    if(loggedIn){
    ...
    if(loggedIn||driverLogged)_registerPushLater();

    if(mounted){
      setState((){
        hasSession=loggedIn;
        restoring=false;
        if(ownerNeedsRelogin)loginMode=true;
      });
    }
    if(loggedIn||driverLogged)Future.delayed(const Duration(milliseconds:500),_openPendingPush);
  }catch(e,st){
    debugPrint('Session restore failed: $e');
    await CrashReporting.record(e,st,reason:'session_restore');
    if(mounted)setState((){hasSession=false;restoring=false;});
  }
}
