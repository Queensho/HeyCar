...
class _OwnerDashboardLiveState extends State<OwnerDashboardLive> {
  ...
  Widget build(BuildContext context) {
    final screens = <Widget>[_OwnerHome(onOpenNotifications: () => setState(() => current = 1)), const OwnerNotificationsPage(), const app.VehiclesSettingsPage(), const app.SettingsPage()];
  ...
  Widget build(BuildContext context) {
    ...
    return ListView(padding: EdgeInsets.zero, children: [
      ...
      Padding(padding: const EdgeInsets.fromLTRB(10, 14, 10, 26), child: Column(children: [
        _Card(child: Row(children: [_brandLogo(), const SizedBox(width: 14), Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text(plate, style: const TextStyle(color: Colors.white, fontSize: 18, fontWeight: FontWeight.w900)), const SizedBox(height: 2), Text(carName, style: const TextStyle(color: _muted, fontSize: 13))])), const Icon(Icons.chevron_right_rounded, color: Colors.white70, size: 28)])),
        const SizedBox(height: 12),
        const Row(children: [Expanded(child: _Stat(icon: Icons.notifications_active_rounded, value: '3', label: 'Yeni\nBildirim', color: Color(0xFFFF4D63))), SizedBox(width: 8), Expanded(child: _Stat(icon: Icons.chat_bubble_outline_rounded, value: '12', label: 'Toplam\nMesaj', color: _purple)), SizedBox(width: 8), Expanded(child: _Stat(icon: Icons.location_on_outlined, value: '5', label: 'Konum\nPaylaşımı', color: Color(0xFF42A5FF))), SizedBox(width: 8), Expanded(child: _Stat(icon: Icons.phone_in_talk_outlined, value: '2', label: 'Arama\nTalebi', color: _purple))]),
