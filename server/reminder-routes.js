...
module.exports = function registerReminderRoutes(app, pool) {
  ...
  async function schema() {
    ...
    await pool.query(`
      CREATE TABLE IF NOT EXISTS vehicle_reminders (
        ...
        UNIQUE(owner_id, vehicle_id, type)
      );
      ...
      CREATE INDEX IF NOT EXISTS idx_vehicle_reminders_due ON vehicle_reminders(due_date) WHERE enabled=TRUE;
    `);
    ready = true;
  }
  ...
  app.put('/api/vehicles/:vehicleId/reminders/:type', express.json(), async (req,res) => {
    ...
    try{
      ...
      await pool.query(`DELETE FROM vehicle_reminder_deliveries WHERE reminder_id=$1`,[r.rows[0].id]);
  ...
  async function sendDueReminders() {
    try{
      ...
      for(const x of r.rows){
        ...
        const claim=await pool.query(`INSERT INTO vehicle_reminder_deliveries(reminder_id,days_before) VALUES($1,$2) ON CONFLICT DO NOTHING RETURNING reminder_id`,[x.id,days]);
        ...
        try{
          ...
          await pool.query(`DELETE FROM vehicle_reminder_deliveries WHERE reminder_id=$1 AND days_before=$2`,[x.id,days]);
        }
      }
