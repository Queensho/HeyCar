module.exports = function registerConversationRoutes(app, pool) {
  async function ensureStatusColumns() {
    await pool.query(`ALTER TABLE qr_conversations ADD COLUMN IF NOT EXISTS status TEXT NOT NULL DEFAULT 'active'; ALTER TABLE qr_conversations ADD COLUMN IF NOT EXISTS closed_at TIMESTAMPTZ;`);
  }
  ...
  app.post('/api/qr/:token/conversations', async (req, res) => {
    ...
    try {
      ...
      const c = await pool.query(`INSERT INTO qr_conversations(vehicle_id, qr_token, notification_id, guest_token, status) VALUES($1,$2,$3,$4,'active') ON CONFLICT(notification_id) DO UPDATE SET updated_at=NOW(), status='active', closed_at=NULL RETURNING id, guest_token, created_at, status`, [q.rows[0].vehicle_id, token, notificationId, guestToken]);
      const initial = String(req.body?.message || '').trim().slice(0, 1000);
      if (initial) {
        const exists = await pool.query(`SELECT 1 FROM qr_conversation_messages WHERE conversation_id=$1 LIMIT 1`, [c.rows[0].id]);
        if (!exists.rows.length) await pool.query(`INSERT INTO qr_conversation_messages(conversation_id, sender, message) VALUES($1,'guest',$2)`, [c.rows[0].id, initial]);
      }
  ...
  app.get('/api/qr/:token/conversations/:id', async (req, res) => {
    ...
    try {
      await ensureStatusColumns();
      const c = await pool.query(`SELECT id,status FROM qr_conversations WHERE id=$1 AND qr_token=$2 AND guest_token=$3 LIMIT 1`, [id, token, guestToken]);
      if (!c.rows.length) return res.status(404).json({ error: 'NOT_FOUND' });
      if (c.rows[0].status === 'closed') return res.status(410).json({ error: 'CONVERSATION_CLOSED' });
  ...
  app.get('/api/owner/notifications/:notificationId/conversation', async (req, res) => {
    ...
    try {
      await ensureStatusColumns();
      const c = await pool.query(`SELECT c.id,c.status FROM qr_conversations c JOIN vehicles v ON v.id=c.vehicle_id WHERE c.notification_id=$1 AND v.owner_id=$2 LIMIT 1`, [notificationId, ownerId]);
      if (!c.rows.length) return res.status(404).json({ error: 'NOT_FOUND' });
      return res.json({ ok: true, conversationId: c.rows[0].id, status: c.rows[0].status });
    } catch (e) { console.error(e); return res.status(500).json({ error: 'SERVER_ERROR' }); }
  });
  ...
  app.get('/api/owner/conversations/:id', async (req, res) => {
    ...
    try {
      await ensureStatusColumns();
      const c = await pool.query(`SELECT c.id,c.status FROM qr_conversations c JOIN vehicles v ON v.id=c.vehicle_id WHERE c.id=$1 AND v.owner_id=$2 LIMIT 1`, [id, ownerId]);
      if (!c.rows.length) return res.status(404).json({ error: 'NOT_FOUND' });
      const m = await pool.query(`SELECT id,sender,message,created_at FROM qr_conversation_messages WHERE conversation_id=$1 ORDER BY created_at ASC`, [id]);
      return res.json({ ok: true, status: c.rows[0].status, messages: m.rows });
    } catch (e) { console.error(e); return res.status(500).json({ error: 'SERVER_ERROR' }); }
  });
  ...
  app.post('/api/owner/conversations/:id/messages', async (req, res) => {
    ...
    try {
      ...
      if (!c.rows.length) return res.status(404).json({ error: 'NOT_FOUND' });
      if (c.rows[0].status === 'closed') return res.status(410).json({ error: 'CONVERSATION_CLOSED' });
      const m = await pool.query(`INSERT INTO qr_conversation_messages(conversation_id,sender,message) VALUES($1,'owner',$2) RETURNING id,sender,message,created_at`, [id, message]);
      await pool.query(`UPDATE qr_conversations SET updated_at=NOW() WHERE id=$1`, [id]);
      return res.status(201).json({ ok: true, message: m.rows[0] });
    } catch (e) { console.error(e); return res.status(500).json({ error: 'SERVER_ERROR' }); }
  });
