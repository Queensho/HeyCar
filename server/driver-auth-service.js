...
function verify(token){
  try{
    ...
    if(p.typ!=='driver'||!p.sub||!p.exp||p.exp<=Math.floor(Date.now()/1000))return null;
    return p;
  }catch(_){return null;}
}
...
function driverId(req){
  ...
  return p?String(p.sub):'';
}
async function rotateRefresh(db,refresh){
...
async function revokeRefresh(db,refresh){
  if(!refresh)return;
  await db.query('UPDATE driver_auth_sessions SET revoked_at=now() WHERE refresh_token_hash=$1',[hash(refresh)]);
}
async function revokeAll(db,driverId){
  await db.query('UPDATE driver_auth_sessions SET revoked_at=now() WHERE driver_id=$1 AND revoked_at IS NULL',[driverId]);
}
module.exports={issueTokens,rotateRefresh,revokeRefresh,revokeAll,driverId,verify,ACCESS_TTL_SECONDS,REFRESH_TTL_DAYS};
